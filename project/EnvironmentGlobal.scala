...
object EnvironmentGlobal {
  ...
  var configSearchMethods: List[(String, String) => File] = List(
    //    check in etc
    (filename: String, scope: String) => File(s"/etc/$scope/$appName/$filename"),
    //    check in config
    (filename: String, scope: String) => File(s"config/$filename"),
    //    check in config+app
    (filename: String, scope: String) => File(s"config/$appName/$filename"),
    //    check in ../config
    (filename: String, scope: String) => File(s"../config/$filename"),
    //    check in ../config+app
    (filename: String, scope: String) => File(s"../config/$appName/$filename")
