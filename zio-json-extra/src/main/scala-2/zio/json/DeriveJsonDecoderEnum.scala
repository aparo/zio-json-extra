...
object DeriveJsonDecoderEnum {
  ...
  def join[A](ctx: CaseClass[JsonDecoder, A]): JsonDecoder[A] = {
    ...
    val enumValue: String = {
      ...
      ctx.annotations.collectFirst { case _: jsonEnumLowerCase =>
        result = result.toLowerCase
        ()
      }
      ctx.annotations.collectFirst { case _: jsonEnumUpperCase =>
  ...
  def split[A](ctx: SealedTrait[JsonDecoder, A]): JsonDecoder[A] = {
    lazy val isLower = ctx.annotations.collectFirst { case _: jsonEnumLowerCase =>
      ()
    }.isDefined

    lazy val isUpper = ctx.annotations.collectFirst { case _: jsonEnumUpperCase =>
      ()
    }.isDefined
