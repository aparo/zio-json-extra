...
package object ast {
  implicit final class JsonOps(private val json: Json) extends AnyVal {
     ...
     *
     * Objects are merged by key, values from the argument JSON take precedence
     * over values from this JSON. Nested objects are recursed.
     *
     * Null, Boolean, String and Number are treated as values, and values from
     * the argument JSON completely replace values from this JSON.
     *
     * `mergeMode` controls the behavior when merging two arrays within JSON.
     * The Default mode treats Array as value, similar to Null, Boolean, String
     * or Number above. The Index mode will replace the elements in this JSON
     * array with the elements in the argument JSON at corresponding position.
     * The Concat mode will concatenate the elements in this JSON array and the
     * argument JSON array.
     */
    def deepMerge(that: Json, mergeMode: MergeMode = MergeMode.Default): Json =
      (json, that) match {
        case (Json.Obj(lhs), Json.Obj(rhs)) =>
          ...
          Json.Obj(
            Chunk.fromIterable(lhs.foldLeft(rhsMap) { case (acc, (key, value)) =>
      ...
      json match {
        case Json.Obj(fields) =>
          (fields.find(_._1 == name) match {
            case Some(value) => Chunk(value._2)
            case None        => Chunk.empty[Json]
          }) ++ fields.flatMap(v => v._2.findAllByKey(name))
        case Json.Arr(elements) => elements.flatMap(_.findAllByKey(name))
